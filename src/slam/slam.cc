...
Pose SLAM::ApplyCSM(LaserScan scan) {
	...
	for (const Pose &pose : possible_poses_){
		...
		// Factor in motion model likelihood
		pose_cost += motion_model_weight*motion_model_prob_[pose_index];
		pose_index++;

		if (pose_cost > max_cost){
			best_pose = pose;
			max_cost = pose_cost;
			CSM_cost = pose_cost - motion_model_weight*motion_model_prob_[pose_index];
			MM_cost = motion_model_weight*motion_model_prob_[pose_index];
		}
	}
...
void SLAM::ApplyMotionModel(Eigen::Vector2f loc, float angle, float dist_traveled, float angle_diff) {
	...
	motion_model_prob_.clear();
	// number of entries will be x_res*y_res*t_res
	float x_res = 13;
	float y_res = 7;
	float t_res = 31;	// theta

	// Noise constants to tune
	const float k1 = 0.80; // translation error per unit translation (suggested: 0.1-0.2)
	const float k2 = 0.10; // translation error per unit rotation    (suggested: 0.01)
	const float k3 = 0.40; // angular error per unit translation     (suggested: 0.02-0.1)
	const float k4 = 1.00; // angular error per unit rotation        (suggested: 0.05-0.2)
	...
	const float t_stddev = k3*dist_traveled + k4*abs_angle_diff;
	...
	{
		float x_noise = x_stddev*(2*x_i/(x_res-1)-1);
		// float x_noise = 0; //odom only
		for (int y_i=0; y_i<y_res; y_i++)
		{
			float y_noise = y_stddev*(2*y_i/(y_res-1)-1);
			// float noise_x = 0; //odom only
			for (int t_i=0; t_i<t_res; t_i++)
			{
				float t_noise = t_stddev*(2*t_i/(t_res-1)-1);
				// float noise_ang = 0; //odom only
				...
				float likelihood = -(x_noise*x_noise)/(x_stddev*x_stddev) 
								   -(y_noise*y_noise)/(y_stddev*y_stddev) 
								   -(t_noise*t_noise)/(t_stddev*t_stddev);
				motion_model_prob_.push_back(likelihood);
			}
		}
	}
}
